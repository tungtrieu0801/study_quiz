                ...
                return (
                    ...
                    >
                        ...
                        {showCheck && isReviewing && (
                            <div className="absolute -top-2 -right-2 bg-white text-green-600 rounded-full border border-green-200 shadow-sm p-0.5">
                                <CheckCircleFilled />
                            </div>
                        )}
                    </motion.button>
                            ...
                            <span className="inline-block mx-1 align-baseline relative group">
                                {isReviewing ? (
                                    ...
                                    <span className="inline-flex flex-col items-center align-middle mx-1">
                                        ...
                                        {userAnswers[index]?.trim().toLowerCase() !== String(correctAnswers[index])?.trim().toLowerCase() && (
                                            <span className="absolute -bottom-6 left-0 whitespace-nowrap text-xs text-white bg-green-600 px-1.5 py-0.5 rounded shadow-sm z-10">
                                    ...
                                    <>
                                        <input
                                            ...
                                            onChange={(e) => onChange(q._id, { index: index, text: e.target.value }, 'FILL_IN_THE_BLANK')}
                                        ...
                                        <span className="absolute -bottom-5 left-1/2 -translate-x-1/2 text-[10px] text-slate-300 select-none">
                                            ({index + 1})
                                        </span>
                                    </>
                                )}
            ...
            {q.options.map((opt) => {
                ...
                if (!isReviewing) {
                    ...
                    if (correctArr.includes(opt)) {
                        ...
                        iconRender = <CloseCircleFilled className="text-red-500 text-xl ml-auto" />;
                    } else { containerClass = "border-slate-100 opacity-40"; }
                }
                return (
        ...
        {isReviewing ? (
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div className="mb-2">
                    ...
                    <div className={`mt-1 font-medium text-lg ${resultData?.isCorrect ? 'text-green-700' : 'text-red-700'}`}>{userAnswer || "(Bỏ trống)"}</div>
                </div>
                {!resultData?.isCorrect && (
                    <div className="mt-3 pt-3 border-t border-slate-200">
                        <span className="text-slate-500 text-sm">Đáp án tham khảo:</span>
                        <div className="mt-1 font-medium text-lg text-green-700">{Array.isArray(resultData?.correctAnswer) ? resultData.correctAnswer.join(" / ") : resultData?.correctAnswer}</div>
                    </div>
                )}
            </div>
        ) : (
            <TextArea rows={3} placeholder="Nhập câu trả lời..." value={userAnswer || ""} onChange={(e) => onChange(q._id, e.target.value, 'SHORT_ANSWER')} className="rounded-xl border-slate-300 text-base focus:border-blue-500 focus:shadow-sm" />
        )}
    </div>
);
...
const QuestionItem = React.forwardRef(({ q, index, userAnswer, resultData, isReviewing, onChange }, ref) => {
    const renderInput = () => {
        switch (q.type) {
            case 'MULTIPLE_SELECT': return <RenderMultipleSelect q={q} userAnswer={userAnswer} resultData={resultData} isReviewing={isReviewing} onChange={onChange} />;
            case 'TRUE_FALSE': return <RenderTrueFalse q={q} userAnswer={userAnswer} resultData={resultData} isReviewing={isReviewing} onChange={onChange} />;
            case 'FILL_IN_THE_BLANK': return <RenderFillInBlank q={q} userAnswer={userAnswer} resultData={resultData} isReviewing={isReviewing} onChange={onChange} />;
            case 'SHORT_ANSWER': return <RenderShortAnswer q={q} userAnswer={userAnswer} resultData={resultData} isReviewing={isReviewing} onChange={onChange} />;
            default: return <RenderSingleChoice q={q} userAnswer={userAnswer} resultData={resultData} isReviewing={isReviewing} onChange={onChange} />;
        }
    };

    return (
        <div ref={ref} className={`p-5 md:p-8 rounded-2xl bg-white shadow-sm border transition-all scroll-mt-48 ${isReviewing ? (resultData?.isCorrect ? 'border-green-200 bg-green-50/20' : 'border-red-200 bg-red-50/20') : 'border-slate-200 hover:shadow-md'}`}>
            <div className="flex gap-4 mb-4">
                ...
                <div className="pt-1 w-full">
                    {/* KHÔNG HIỆN CONTENT Ở ĐÂY CHO DẠNG ĐIỀN TỪ, VÌ ĐÃ HIỆN TRONG INPUT RỒI */}
                    {q.type !== 'FILL_IN_THE_BLANK' && (
                        <h3 className="text-base md:text-lg font-semibold text-slate-800 leading-relaxed mb-3">{q.content}</h3>
                    )}
                    {q.type === 'FILL_IN_THE_BLANK' && (
                        <div className="mb-2">
                             <span className="inline-block px-2 py-1 bg-blue-50 text-blue-600 text-[10px] font-bold uppercase rounded tracking-wider border border-blue-100">
                                Điền từ vào chỗ trống
                             </span>
                        </div>
                    )}
                    {q.imageUrl && (
                        <div className="w-full my-4 flex justify-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <Image src={q.imageUrl} className="rounded-lg object-contain mix-blend-multiply" style={{ maxHeight: "300px", maxWidth: "100%" }} />
                        </div>
                    )}
                </div>
            </div>
            ...
            {isReviewing && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-6 ml-0 md:ml-14 p-4 bg-amber-50 rounded-xl border border-amber-200 text-slate-700 text-sm md:text-base">
                    ...
                    <div className="leading-relaxed">
                        {resultData?.solution || (resultData ? <span className="italic text-slate-500">Không có lời giải chi tiết.</span> : <span className="italic text-slate-400">Hệ thống không hiển thị đáp án cho câu hỏi chưa làm.</span>)}
                    </div>
                </motion.div>
            )}
        </div>
